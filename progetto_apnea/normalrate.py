...
def create_windowed_dataset(bcg_signals, probabilities, window_size=30, step_size=1, sampling_rate=1000):
...
def compute_apnea_probability(ecg_signal, sampling_rate=1000):
    # Adattato per 1kHz: distanza minima tra picchi aumentata a 600ms
...
def plot_apnea_analysis(probabilities, y, step_size=1):
    ...
    time = np.arange(len(probabilities)) / 1000  # Adattato per 1kHz
...
if __name__ == "__main__":
    csv_file = "c:\\Users\\domen\\OneDrive\\Desktop\\Template\\apps\\data\\ecg_bcg1_indexed_clean5.csv"
    
    print("Caricamento dati...")
    bcg_signals, ecg_signal = load_signals(csv_file, chunk_size)
    
    print("Calcolo probabilità di apnea...")
    probabilities, _ = compute_apnea_probability(ecg_signal, sampling_rate)
    
    print("Creazione dataset con finestre...")
    X, y = create_windowed_dataset(bcg_signals, probabilities, window_size, step_size, sampling_rate)
    
    print("Creazione grafico di analisi...")
    plot_apnea_analysis(probabilities, y, step_size)
    
    print(f"Dataset creato con successo!")
    print(f"- Numero finestre: {X.shape[0]}")
    print(f"- Dimensione finestre: {X.shape}")
    print(f"- Frequenza campionamento: {sampling_rate} Hz")